...
class DeepEmotionRecognizer(EmotionRecognizer):
    ...
    def _compute_input_length(self):
        ...
        if model_name:
            ...
            self.input_length = self.model.layers[0].input_shape[2]
            if self.verbose > 0:
                print("[*] Model loaded")
            return
    ...
    def train(self, override=False):
        ...
        """
        if not override and self.model_trained:
            return
    ...
    def confusion_matrix(self, percentage=True, labeled=True):
        ...
        matrix = confusion_matrix(
            y_test, y_pred, labels=[self.emotions2int[e] for e in self.emotions]
        ).astype(np.float32)
        if percentage:
...
if __name__ == "__main__":
    rec = DeepEmotionRecognizer(
        emotions=["angry", "sad", "neutral", "ps", "happy"], epochs=300, verbose=0
    )
